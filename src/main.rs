...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match !from_state.is_empty() && !to_state.is_empty() {
        true => {
            info!(
    ...
    let obj = json!({
        ...
        "beginning_zipcode":from_zipcode,
        "ending_state":to_state.to_string(),
        "ending_zipcode":to_zipcode,
